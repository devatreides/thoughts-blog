...
const Programacao = ({ data, title, description }) => {
  ...
  return (
    <>
      ...
      <Navbar hrefReturn="/" articleMetaData={{ title: 'Games' }} />
              ...
              <div className="md:w-3/5 p-4 md:h-40">
                <div className="mb-8">
                  <div className="text-accent-1 font-bold text-xl mb-2">
                    <Link href={`/games/${blog.slug}`}>{blog.title}</Link>
                  </div>
...
export async function getStaticProps() {
  const files = fs.readdirSync(`${process.cwd()}/content/games`, 'utf-8');

  const blogs = files.filter((fn) => fn.endsWith('.md'));
  const data = blogs.map((blog) => {
    const path = `${process.cwd()}/content/games/${blog}`;
